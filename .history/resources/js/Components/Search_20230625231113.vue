                                ...
                                <tr class="border-b border-slate-100 hover:bg-gray-100 group odd:bg-slate-200"
                                    ...
                                    <template v-for="(value, key) in user">
                                        <td class="px-4 py-3 text-sm font-medium text-gray-500"
                                            v-if="!rejectedKeys.includes(key)">{{ value }}
                                        </td>
...
const { getUsers, users, getDataTable, dataTable } = useUserServices()
    ...
    if (data.length > 0) {
        const result = Object.keys(data[0]).filter(
            (key) => {
                ...
                if (typeof (data[0]?.[key]) == 'object' && data[0]?.[key]?.length > 0) {
                    console.log(data[0]?.[key], 'rasult6');
                   return Object.keys(data[0]?.[key][0])
