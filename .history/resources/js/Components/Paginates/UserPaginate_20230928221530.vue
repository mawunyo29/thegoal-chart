...
onMounted(async () => {
    ...
    new Promise((resolve, reject) => {
        getPaginateUsers(cursor.value)
            .then((response) => {
                if (response.data.success) {
                    data.value = response.data.users.data;
                    cursor.value = response.data.users.next_cursor;
                    totalPages.value = response.data.users.lastPage;
