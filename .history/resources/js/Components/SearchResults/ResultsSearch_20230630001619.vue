...
const rejectedKeys = reactive(props.rejectedKeys);
const data = computed(() => {
    return props.data;
});
...
const filteredData = computed(() => {
    return data.value.filter((item) => {
        return !rejectedKeys.includes(item);
    });
});

const sort = (key) => {
    if (rejectedKeys.includes(key)) {
        rejectedKeys = rejectedKeys.filter((item) => {
            return item !== key;
        });
    } else {
...
const reset = () => {
    ...
    if(rejectedKeys.length === 0){
        console.log("empty" ,props.rejectedKeys);
    }
    // rejectedKeys.value =props.rejectedKeys;
    console.log(rejectedKeys, 'reset');
};
