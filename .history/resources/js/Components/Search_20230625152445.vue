                ...
                <div class="flex flex-col w-full bg-white rounded-md shadow-lg max-h-60 ">
                    ...
                    <div class="overflow-y-auto h-60">
                        <table class="w-full">
                            ...
                            <tbody>
                                <tr class="border-b border-slate-200 hover:bg-gray-100 group odd:bg-slate-300" v-for="(user ,index ,value) in filteredData"
                                    :key="user.id">
                                    <template v-for="(value, key) in user">
                                        <td class="px-4 py-3 text-sm font-medium text-gray-500" v-if="!rejectedKeys.includes(key)">{{ value }}</td>
                                    </template>                             </tr>
                            </tbody>
                        </table>
                    </div>
