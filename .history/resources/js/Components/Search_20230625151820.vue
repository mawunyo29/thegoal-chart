                        ...
                        <table class="w-full">
                            ...
                            <tbody>
                                <tr class="border-b border-gray-200 hover:bg-gray-100" v-for="(user ,index ,value) in filteredData"
                                    :key="user.id">
                                    <td class="px-4 py-3 text-sm font-medium text-gray-500">{{ filteredData}}</td>
...
const filteredData = computed(() => {
    ...
    if (searchInput.value.length > 0) {
        return Object.values(users.value.data).filter((item) =>
            Object.values(item)
                .filter((item) => !rejectedKeys.includes(item))
                .join('')
                .toLowerCase()
