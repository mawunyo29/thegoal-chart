                            ...
                            <tbody>
                                <tr class="border-b border-slate-100 hover:bg-gray-100 group odd:bg-slate-200"
                                    ...
                                    <template v-for="(value, key) in user">
                                        <template v-if="!rejectedKeys.includes(key) && (typeof (value) !== 'object')"
                                            :key="key">
                                        ...
                                        <template v-if="!rejectedKeys.includes(key) && (typeof (value) === 'object')"
                                            :key="key">
                                            <template v-for="(relation ,indexkey) in value" >
                                                <td class="px-4 py-3 text-sm font-medium text-gray-500"
                                                    v-if="!rejectedKeys.includes(key) && (typeof (value) === 'object')"
                                                    v-text="Object.values(relation)[Object.keys(relation)]"></td>
...
const headings = computed(() => {
    ...
    if (data.length > 0) {
        if (Object.values(data[0]).some((value) => typeof (value) === 'object')) {
