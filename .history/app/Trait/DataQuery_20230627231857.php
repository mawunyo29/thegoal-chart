...
trait DataQuery
{
    ...
    public function scopeModelFilterWithRelations( array $filters): Builder
    {
        ...
        $query->when($filters['relation'] ?? null, function (Builder $query, string $relation) use ($data) {
            $query->whereHas($relation, function (Builder $query) use ($relation, $data) {
                $query->when($filters['search'] ?? null, function (Builder $query, string $search) use ($data, $relation) {
                    $query->where(function (Builder $query) use ($search, $data, $relation) {
