...
trait DataQuery
{
    ...
    public function scopeModelFilterWithRelations(array $filters): Builder
    {
       ...
       $result= $query->when($filterSearch, function (Builder $query, string $search) use ($columns) {
            $query->where(function (Builder $query) use ($search, $columns) {
                foreach ($columns as $column) {
      ...
      $relationresult=  $query->when($filters['relation'] ?? null, function (Builder $query, string $relation) use ($data, $filters) {
                ...
                $query->when($filters['realtionsearch'] ?? null, function (Builder $query, string $realtionsearch) use ($data, $relation) {
                    $query->where(function (Builder $query) use ($realtionsearch, $data, $relation) {
                        foreach ($data[$relation] as $column) {
                            $column = $this->resolveColumn($column);
                            $query->orWhere($column, 'like', '%' . $realtionsearch . '%');
                        }
                    });
                });
            });
        ...
        $result = $query->union($relationresult);
        return $result;
    ...
    public function resolveColumn($column)
    {
        ...
        if (in_array($column, $columns)) {
            ...
            $relations = $this->getRelations();
            foreach ($relations as $relation) {
                $relationColumns = DB::getSchemaBuilder()->getColumnListing($relation);
